...
<html>
	<head>
		...
		function validar_campos(){
            if(trim(document.getElementById("titulo").value).length == 0){ // Valido que el titulo no viaje vacío
                alert('Debe ingresar el título del video'); return 0;}
            if(trim(document.getElementById("url").value).length == 0){ // Valido que la url no viaje vacía
                alert('Debe ingresar la dirección URL del video'); return 0;}
			// Envio el formulario
	...
	<body>
        ...
        $consulta="select MAX(id) from videos"; // Obtiene el máximo índice de la tabla
